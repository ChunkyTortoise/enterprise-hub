...
def render() -> None:
    ...
    try:
        ...
        with col2:
            _render_results(
                contribution_margin, contribution_margin_ratio, break_even_units, break_even_revenue,
                margin_of_safety_pct, margin_of_safety_units, operating_leverage, current_profit,
                target_units, current_sales_units, unit_price, unit_cost, fixed_costs, target_revenue,
                target_profit
            )
...
def _render_results(
    ...
    m1.metric("Contribution Margin", f"${contribution_margin:.2f}", f"{contribution_margin_ratio:.1f}%")
    m2.metric("Break-Even Units", f"{int(np.ceil(break_even_units)):,} units")
    m3.metric("Break-Even Revenue", f"${break_even_revenue:,.2f}")
        
    m4, m5, m6 = st.columns(3)
    m4.metric("Margin of Safety", f"{margin_of_safety_pct:.1f}%", f"{int(margin_of_safety_units):,} units")
    m5.metric("Operating Leverage", f"{operating_leverage:.2f}x")
    m6.metric("Current Profit", f"${current_profit:,.2f}")

    # --- CVP Visualization ---
    _render_cvp_chart(target_units, current_sales_units, break_even_units, unit_price, unit_cost, fixed_costs, break_even_revenue)
    
    # --- Sensitivity Analysis ---
    _render_sensitivity_heatmap(unit_price, unit_cost, current_sales_units, fixed_costs)
    
    # --- Scenario Table & Export ---
    _render_scenario_table(break_even_units, break_even_revenue, current_sales_units, unit_price, current_profit, target_units, target_revenue, target_profit)


def _render_cvp_chart(target_units, current_sales_units, break_even_units, unit_price, unit_cost, fixed_costs, break_even_revenue):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=units_range, y=units_range * unit_price, name="Revenue", line=dict(color="#00ff88", width=3)))
    fig.add_trace(go.Scatter(x=units_range, y=fixed_costs + (units_range * unit_cost), name="Total Cost", line=dict(color="#ff4444", width=3), fill='tonexty', fillcolor='rgba(255, 68, 68, 0.1)'))
    fig.add_trace(go.Scatter(x=units_range, y=[fixed_costs] * len(units_range), name="Fixed Costs", line=dict(color="#888888", dash="dash")))
    fig.add_trace(go.Scatter(x=[break_even_units], y=[break_even_revenue], mode="markers", name="Break-Even Point", marker=dict(color="white", size=12, symbol="star")))
    fig.add_trace(go.Scatter(x=[current_sales_units], y=[current_sales_units * unit_price], mode="markers", name="Current Sales", marker=dict(color="#00D9FF", size=10, symbol="circle")))

    fig.update_layout(title="Revenue vs Costs (Green=Profit, Red=Loss)", xaxis_title="Units Sold", yaxis_title="Amount ($)", template="plotly_dark", height=400, hovermode="x unified")
...
def _render_sensitivity_heatmap(unit_price, unit_cost, current_sales_units, fixed_costs):
    ...
    z_data = [[(p - c) * current_sales_units - fixed_costs for p in price_range] for c in cost_range]
        
    fig_heat = go.Figure(data=go.Heatmap(z=z_data, x=[f"${p:.2f}" for p in price_range], y=[f"${c:.2f}" for c in cost_range], colorscale='RdBu', zmid=0, colorbar=dict(title="Net Profit ($)")))
    fig_heat.update_layout(title="Profit Sensitivity Matrix", xaxis_title="Unit Price ($)", yaxis_title="Variable Cost ($)", template="plotly_dark", height=400)
    st.plotly_chart(fig_heat, use_container_width=True)

def _render_scenario_table(break_even_units, break_even_revenue, current_sales_units, unit_price, current_profit, target_units, target_revenue, target_profit):
    ...
    st.download_button(label="📥 Download Scenarios CSV", data=csv, file_name='margin_hunter_scenarios.csv', mime='text/csv')
