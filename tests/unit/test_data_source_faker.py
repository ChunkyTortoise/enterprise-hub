...
def test_generate_campaign_data_structure():
    ...
    expected_columns = [
        "Date", "Platform", "Impressions", "Clicks", "Spend",
        "Conversions", "Revenue", "CPC", "CTR", "Conversion_Rate", "ROAS"
    ]
...
def test_generate_campaign_data_date_range():
    ...
    assert df['Date'].max().strftime("%Y-%m-%d") == (datetime.strptime(start, "%Y-%m-%d") + timedelta(days=days-1)).strftime("%Y-%m-%d")
...
def test_generate_campaign_data_values_plausibility():
    ...
    df = generate_campaign_data(
        platform="Google Ads", days=10,
        base_impressions=10000, base_cpc=1.0, base_ctr=0.02,
        base_conversion_rate=0.05, conversion_value=10.0
    )
...
def test_generate_campaign_data_no_negative_values():
    ...
    for _ in range(10): # Run multiple times due to randomness
        df = generate_campaign_data(platform="Google Ads", days=100,
                                     base_impressions=100, base_cpc=0.1,
                                     base_ctr=0.001, base_conversion_rate=0.001)
        assert (df['Impressions'] >= 0).all()
        assert (df['Clicks'] >= 0).all()
        assert (df['Spend'] >= 0).all()
        assert (df['Conversions'] >= 0).all()
